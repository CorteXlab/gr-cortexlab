...
unsigned char decode_chips(unsigned int chips){
	...
	for(i=0; i<16; i++) {
		...
		unsigned int threshold =  count_bits32((chips & 0x7FFFFFFE) ^ (CHIP_MAPPING[i] & 0x7FFFFFFE));
...
ieee802_15_4_packet_sink_impl(int threshold)
...
ieee802_15_4_packet_sink::sptr ieee802_15_4_packet_sink::make(unsigned int threshold) {
	return gnuradio::get_initial_sptr(new ieee802_15_4_packet_sink_impl(threshold));
}
